...
impl PPUMemoryMap {
    ...
    pub fn write(&mut self, mapper: &mut Box<dyn Mapper>, addr: u16, data: u8) {
        ...
        match addr {
            ...
            0x2000..=0x2FFF => {
                self.vram[nametable_addr(mapper.nametable_arrangement(), addr) as usize] = data;
            }
            // Address space typically(?) mirrors the above address space
            0x3000..=0x3EFF => {
                self.vram
                    [nametable_addr(mapper.nametable_arrangement(), addr & !(1 << 12)) as usize] =
                    data;
            }
            0x3F00..=0x3FFF => {
                self.pallette_ram[palette_index(addr)] = data;
            }
            _ => (),
        };
    }
    ...
    pub fn read(&self, mapper: &mut Box<dyn Mapper>, addr: u16) -> u8 {
        let ret = match addr {
            ...
            0x2000..=0x2FFF => {
                self.vram[nametable_addr(mapper.nametable_arrangement(), addr) as usize]
            }
            // Address space typically(?) mirrors the above address space
            0x3000..=0x3EFF => {
                self.vram[nametable_addr(mapper.nametable_arrangement(), addr) as usize]
