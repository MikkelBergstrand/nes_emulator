...
pub const F_CPU: f32 = 1_789_773.0;
...
impl NES {
    ...
    pub fn from_args() -> Self {
        let rom_file = std::env::args()
            .nth(1)
            .unwrap_or(String::from("roms/smb1.nes"));
        let color_data_file = std::env::args()
            .nth(2)
            .unwrap_or(String::from("colors.pal"));

        let mut color_data: Vec<u8> = std::fs::read(&color_data_file).unwrap();
        color_data.truncate(192);

        let color_data: Vec<Rgb<u8>> = color_data
            .chunks_exact(3)
    ...
    pub fn get_image_bytes(&mut self) -> &Vec<u8> {
        self.ppu.get_image_bytes()
    }

    pub fn image_ready(&self) -> bool {
        return self.ppu.image_ready();
    }
    ...
    /// Total CPU cycles executed since power-on. `tick` runs whole
    /// instructions, so this advances in variable-sized steps; it's the clock
    /// the audio resampler needs to place samples correctly in time.
